...
export function hasTitleIssue(title) {
  const t = title.trim()
  if (!t) return true
...
/** 面向首页展示的标题是否仍带平台/帖子感 */
export function hasPremiumDisplayIssue(title) {
  const t = title.trim()
  if (!t) return true
...
/** 是否为完整菜品（首页精品池仅推荐完整菜） */
export function isGenericFallbackTitle(title) {
  return GENERIC_FALLBACK_TITLES.includes(title.trim())
}

export function isPremiumFullDish(title) {
  const t = title.trim()
  if (!t) return false
...
function stripMarketingTokens(text) {
  let t = text
...
function pickBestSegment(title) {
...
function extractDishFallback(title) {
...
function extractQuotedDish(title) {
...
/** 纯营销标题无菜名时的兜底，避免空标题 */
function resolveBareMarketingTitle(title) {
...
export function hasTimeIssue(recipe) {
...
export function estimateMinTime(recipe) {
  const time = Number(recipe.time) || 0
  const steps = recipe.steps?.length ?? 0
  const ingCount = recipe.ingredients?.length ?? 0
...
function hasIngredientIssue(ing) {
...
function countIngredientIssues(recipe) {
  return (recipe.ingredients || []).filter(hasIngredientIssue).length
}

export { countIngredientIssues }

function countBadStepUrls(recipe) {
  return (recipe.steps || []).filter((s) => s.image && !VALID_IMAGE_RE.test(s.image)).length
}

export { countBadStepUrls }

export function computeQualityScore(recipe, imageDupCount = 1) {
  let score = 100
...
export function enrichRecipeQuality(recipe, imageDupCount = 1) {
...
export function isRecommendable(recipe) {
  return (recipe.qualityScore ?? 100) >= MIN_RECOMMEND_QUALITY
}

/** 首页 / Profile 等面向用户的精品推荐池 */
export function isPremiumDisplayRecipe(recipe) {
  const originalTitle = String(recipe.originalTitle || recipe.title || '')
